...
`69d0ef8`(HIGH-1/HIGH-2 + 재리뷰 A/B/C), `13d58b9`(초대 이메일 검증), `eb6be08`(accuracy_m/
battery_level 입력 검증). 이어서 프론트 P0 6건과 후속 수정/리팩터링도 커밋됐다. 이 문서에서
지적한 항목 중:

> **해시 정정 (2026-09-02, Rena).** 위 세 해시는 이 문서에 원래
> `26e3bed`/`99bef0b`/`5c77b96`로, 프론트 커밋도
> `75dd721`/`a0fb944`/`b8a4e8e`/`2c42820`/`d4909a0`로 적혀 있었다. 이후 히스토리가
> 갈리면서(rebase·재작성) 커밋 SHA만 바뀌고 이 문서의 해시 텍스트만 옛것으로 남았다 — 지적
> 내용과 결론 자체는 그대로 유효하다. 현재 리포 기준 실제 커밋은 `69d0ef8`(20260823100002) /
> `13d58b9`(20260823100001) / `eb6be08`(20260823100003)이다. **옛 해시가 `git show`로 조회되지
> 않는다고 해서 이 문서의 항목을 "미수정"으로 읽지 말 것.** (프론트 커밋 해시는 이번에 다시
> 매핑하지 않았다 — 마찬가지로 갈린 히스토리의 옛 SHA이며, 실제 반영 여부는 아래 "프론트" 줄과
> 각 P0 섹션으로 판단한다.)

- HIGH-1/HIGH-2/A(get_share_mode 그룹경계)/B(off·미설정 유출)/C(일시중지 그룹별 미적용) —
  전부 코드 레벨로 수정 확인·검증 완료(아래 각 섹션 참고).
  - *각주 — 아래 「(1) HIGH-1 / HIGH-2가 실제로 닫혔는가」 섹션의 HIGH-2 부분 정정
    (2026-09-02, Tom 실행 결과 반영):* 그 섹션은 `get_peer_locations`에서 "대상이 그 그룹의
    멤버" + "호출자도 그 그룹의 멤버" **두 조건이 함께 있어야 행이 나온다**고 썼는데, 게이팅의
    무게가 어디에 실려 있는지를 놓쳤다. HIGH-2 #4(비멤버가 남의 `group_id`로 조회 → 빈 결과)는
    **픽스 전후 모두 PASS**다. `get_peer_locations`는 INVOKER 함수라 `relationship_members`에
    대한 RLS가 이미 크로스그룹 읽기를 막고 있어서, 100002가 새로 넣은 `is_group_member(호출자)`
    체크는 이 함수에서는 사실상 중복이고 방어심층으로만 의미가 있다. 진짜 load-bearing한
    크로스그룹 픽스는 SECURITY DEFINER라 RLS를 우회하는 `get_share_mode` 쪽이고, 그건
    #9(픽스 전 FAIL → 후 PASS)가 잡는다. 이 구분 — 크로스그룹 경계를 실제로 지탱하는 지점이
    `get_share_mode` 로직 내부의 멤버십 체크라는 것 — 도 다음 라운드 게이팅 아키텍처 티켓의
    근거가 된다.
...
- `is_location_paused`의 행-존재-여부 오라클 — 검토 결과 **별도 수정 불필요**로 결론(해당 섹션
  ...
  - **재확인 (2026-09-02, Rena — `20260823100002` 커밋본 기준).** 결론 유지. 커밋된 마이그레이션에서
    `get_share_mode`와 `is_location_paused`의 WHERE 절(게이팅)이 글자 그대로 동일함을 재확인했다
    (`s.user_id` / `s.relationship_group_id` 일치 + `is_group_member(group, auth.uid())` +
    `owner = auth.uid() or can_view_location(owner, auth.uid())`). 두 함수가 non-NULL을 내는 입력
    집합이 완전히 같으므로 — (A) 수정으로 양쪽에 `is_group_member`가 똑같이 들어가 문서 작성
    시점보다 전제가 더 단단해졌다 — `is_location_paused`만 단독으로 닫아도 같은 caller가
    `get_share_mode`를 직접 RPC로 호출해 우회한다.
  - *각주 — 아래 「is_location_paused의 "설정 행 존재 여부" 오라클」 섹션 논리 정정:* 그 섹션은
    `is_location_paused`를 `get_share_mode` 노출의 "완전한 부분집합"이라고 표현했으나 엄밀히는
    아니다. `is_location_paused`는 `get_share_mode`가
    주지 않는 "지금 일시중지 중인지" 1비트를 추가로 노출한다. 다만 그 1비트도 같은 caller가
    `get_share_mode`(반환값이 `'precise'`/`'approx'`인지 확인) + `get_peer_locations(그룹)`에서
    해당 상대가 결과에서 빠지는 것을 조합하면 유도할 수 있다(둘 다 이미 승인된 채널). 따라서
    새로운 능력이 아니라 편의 제공일 뿐이고, **"단독 차단은 실익 없음"이라는 결론은 그대로다.**
  - **다음 라운드 티켓의 핵심 — 100002 이후 실제 잔여 노출:** 그룹 B의 진짜 멤버이면서 대상과
    어느 그룹에서든 활성 공유 중인 caller는, 대상이 B에서 모드를 off로 해놨어도 대상의 B 모드값과
    일시중지 상태를 읽을 수 있다. 티켓은 `is_location_paused` 단독이 아니라
    `get_share_mode`/`is_location_paused`가 공유하는 게이팅(크로스그룹 `can_view_location`을 특정
    그룹 한정으로 좁힐지)의 아키텍처 재검토로 다룬다.
- `supabase/tests/database/location_sharing_security.test.sql`이 신설되어 위 6건
  ...
  케이스 구성을 코드로 대조해 시나리오 설계를 검증했고(그룹 경계/크로스그룹 RLS를 피하도록
  픽스처가 짜여 있어 "우연히 통과"가 아니라 실제 로직을 검증하는 구조), **이후 Tom이 실제로
  실행해 결과를 확인했다 (2026-09-02).** Tom이 PostgreSQL 14.24 + PostGIS 3.2를 직접
  설치·기동하고 `gyeote_test` DB에 스텁 + 마이그레이션 15개를 적용해 돌렸다:
  `location_sharing_security.test.sql` **14 PASS / 0 FAIL**,
  `location_sharing.test.sql` **9 PASS**. 원문 출력은 가인님께 전달됨. (재검토서를 처음 쓸 때
  Dexa가 조사한 시점에는 샌드박스에 실제로 `psql`이 없어 실행이 불가했고, 이후 Tom이 PostgreSQL을
  직접 설치해 검증했다. `location_sharing.test.sql` 파일 상단의 "미실행" 표기는 이번 9 PASS
  실행으로 더 이상 맞지 않으니 그 파일을 손댈 때 같이 정리할 것.)
  - **음성 대조군까지 실행돼 픽스가 load-bearing함이 증명됐다.** `20260823100002`/`100003`을
    빼고 나머지 마이그레이션만 적용한 상태로 돌리면 HIGH-1 #1·#2 / C #9 / D #10 / F #13 /
    E #11·#12 총 7건이 FAIL로 잡힌다. 각 케이스가 픽스처 우연이 아니라 실제 픽스에 의존해
    통과함이 실행으로 확인됐다.
- 프론트(`b228b01` 기준): `flutter analyze` 에러 0 / 경고 0 / info 7, `flutter test` 92/92 통과
  (P0-8이 56건 추가, 기준선 36). info 델타 1건은 `supabase_client.dart:19`의 anonKey deprecated
  경고로 **P0-8과 무관**하다 — `pubspec.lock` 추적 시작(`626741c`) 후 fresh pub get이 최신
  패키지를 끌어와 드러난 것이며 P0-8이 건드린 파일이 아니다.
