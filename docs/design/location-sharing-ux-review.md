...
| **위치 수집의 앱 라이프사이클 연동**(`AppLifecycleState` 구독해 백그라운드 진입 시 하트비트 타이머 명시적 취소) | **v0.2** | Rena 지적 — 지금은 OS의 백그라운드 프로세스 정지에만 기대는 상태. 방침 문구를 완화한 이유이기도 함 → `v0.1-release-assets.md` §7 |
